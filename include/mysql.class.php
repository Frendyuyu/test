...
class DbMysql {
    ...
    function connect() {
        if ($this->pconnect) {
            ...
            // 显示具体连接错误信息：mysqli_connect_error()
    ...
    function auto_id($table) {
        $this->query("ANALYZE TABLE " . $this->table($table));
    ...
    // 验证信息是否已经存在
    function value_exist($table, $field, $value, $and = '') {
        $and = $and ? ' AND ' . $and : '';
        $sql = "SELECT * FROM " . $this->table($table) . " WHERE $field = '$value'" . $and;
    ...
    function get_value($table, $field, $where = '') {
        $value = $this->get_one("SELECT $field FROM " . $this->table($table) . " WHERE $where");
        if ($value !== false) {
            return $value;
        } else {
            return false;
    ...
    // 删除数据
    function delete($table, $where, $url = '') {
        if ($this->query("DELETE FROM " . $this->table($table) . " WHERE $where")) {
            if (!empty($url)) {
