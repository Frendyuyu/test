...
elseif ($step == 'check') {
    ...
    $sys_info['mysql_ver'] = extension_loaded('mysqli') ? $_LANG['yes'] : $_LANG['no'];
...
elseif ($step == 'install') {
    ...
    $config_str .= " * 网站地址: https://www.douphp.com\r\n";
    $config_str .= " * --------------------------------------------------------------------------------------------------\r\n";
    $config_str .= " * 这不是一个自由软件！您只能在遵守授权协议前提下对程序代码进行修改和使用；不允许对程序代码以任何形式任何目的的再发布。\r\n";
    $config_str .= " * 授权协议: https://www.douphp.com/license.html\r\n";
    $config_str .= " * --------------------------------------------------------------------------------------------------\r\n";
    $config_str .= " * Author: DouCo Co.,Ltd.\r\n";
    $config_str .= " * Release Date: 2020-06-01\r\n";
    $config_str .= " */\r\n\r\n";
    ...
    $config_str .= "define('SYSTEM_SIGN', 'company');" . "\r\n\r\n";
    ...
    if (!$cue) {
        ...
        } elseif ($_POST['email'] && !$install->is_email($_POST['email'])) {
            $cue = $_LANG['cue_email_wrong'];
        }
    }
    ...
    if (!$cue) {
        ...
        $update_date = 'a:3:{s:6:"system";a:2:{s:6:"update";s:8:"' . $date . '";s:5:"patch";s:8:"' . $date . '";}s:6:"module";a:4:{s:7:"article";s:8:"' . $date . '";s:7:"product";s:8:"' . $date . '";s:3:"box";s:8:"' . $date . '";s:8:"fragment";s:8:"' . $date . '";}s:5:"theme";a:0:{}}';
        
        // 初始化管理员账号
        $dou->fn_execute("UPDATE $table_admin SET user_name = '$username', password = '$password', email = '$email', add_time = '$add_time' WHERE user_id = '1'");
        ...
        // 初始数据
        if ($_POST['test_data'] == 'no') {
            $dou->fn_execute("TRUNCATE " . $prefix . "admin_log");
            $dou->fn_execute("TRUNCATE " . $prefix . "article");
            $dou->fn_execute("TRUNCATE " . $prefix . "article_category");
            $dou->fn_execute("TRUNCATE " . $prefix . "product");
            $dou->fn_execute("TRUNCATE " . $prefix . "product_category");
            $dou->fn_execute("TRUNCATE " . $prefix . "show");
            $dou->fn_execute("TRUNCATE " . $prefix . "file");
            
            $install->del_dir(ROOT_PATH . 'data/slide', true);
            $install->del_dir(ROOT_PATH . 'images', true);
        }
...
elseif ($step == 'finish') {
    ...
    $content .= "single_module:box,fragment\r\n\r\n";
    $content .= "'\r\n";
    $content .= '?>';
