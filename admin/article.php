...
if ($rec == 'default') {
    ...
    // 未加入分页条件的SQL语句
    $sql = "SELECT id, title, cat_id, image, sort, add_time FROM " . $dou->table('article') . $where . " ORDER BY " . $sort . "id DESC";
...
elseif ($rec == 'add') {
    ...
    $article['add_time'] = date('Y-m-d H:i', time());
    ...
    $smarty->assign('article_category', $dou->get_category_nolevel('article_category'));
    $smarty->assign('item_id', $dou->auto_id('article'));
    $smarty->assign('article', $article);
    $smarty->assign('btn_lang', $dou->btn_lang('article', '', 'title, content, keywords, description'));
...
elseif ($rec == 'insert') {
    ...
    // 数据格式化
    $add_time = $_POST['add_time'] ? strtotime($_POST['add_time']) : time();
...
elseif ($rec == 'edit') {
    ...
    $smarty->assign('item_content', $article['content']);
    $smarty->assign('article', $article);
    $smarty->assign('btn_lang', $dou->btn_lang('article', $id, 'title, content, keywords, description'));
...
elseif ($rec == 'update') {
    ...
    // 格式化数据
    if ($_POST['add_time'])
        $add_time = ", add_time = '" . strtotime($_POST['add_time']) . "'";
    ...
    $sql = "UPDATE " . $dou->table('article') . " SET cat_id = '$_POST[cat_id]', title = '$_POST[title]', defined = '$_POST[defined]' ,content = '$_POST[content]'" . $image . ", keywords = '$_POST[keywords]', description = '$_POST[description]', sort = '$_POST[sort]'" . $add_time . " WHERE id = '$id'";
    $dou->query($sql);
