...
if ($rec == 'system') {
    ...
    elseif ($act == 'update') {
        ...
        if ($_FILES['mobile_logo']['name'] != '') {
            ...
            $mobile_logo = $file->upload('mobile_logo', 'logo'); // 上传的文件域：mobile_logo，文件名：logo
    ...
    elseif ($act == 'clear_logo') {
        ...
        if (preg_match("/^[a-z.]+$/", $mobile_logo)) {
            @unlink(ROOT_PATH . M_PATH . '/theme/' . $mobile_theme . '/images/' . $mobile_logo);
            $dou->query("UPDATE " . $dou->table('config') . " SET value = '' WHERE name = 'mobile_logo'");
        }

        $dou->create_admin_log($_LANG['del'] . ': ' . M_PATH . '/theme/' . $mobile_theme . '/images/' . $mobile_logo);
...
elseif ($rec == 'show') {
    ...
    elseif ($act == 'insert') {
        ...
        $sql = "INSERT INTO " . $dou->table('show') . " (id, show_name, show_link, show_img, show_text, type, sort)" . " VALUES (NULL, '$_POST[show_name]', '$_POST[show_link]', '$show_img', '$_POST[show_text]', 'mobile', '$_POST[sort]')";
        $dou->query($sql);
    ...
    elseif ($act == 'update') {
        ...
        $sql = "update " . $dou->table('show') . " SET show_name='$_POST[show_name]', show_link = '$_POST[show_link]'" . $show_img . ", show_text = '$_POST[show_text]', sort = '$_POST[sort]' WHERE id = '$id'";
        $dou->query($sql);
...
elseif ($rec == 'theme') {
    ...
    elseif ($act == 'enable') {
        if ($check->is_extend_id($unique_id = $_REQUEST['unique_id'])) {
            $theme_array = $dou->get_subdirs(ROOT_PATH . M_PATH . '/theme/');
            if (in_array($unique_id, $theme_array)) { // 判断模板是否存在
                if ($unique_id != 'default')
                    $dou->copy_dir(ROOT_PATH . M_PATH . '/theme/default', ROOT_PATH . M_PATH . '/theme/' . $unique_id, false, true);
